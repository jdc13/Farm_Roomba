...
#Functions outside the class that may be useful in camera operation
def nothing(blank):
    pass
...
def crop(image,bnd):
    ...
    image[0:height-bnd[0],:] = 0
    image[:,0:bnd[2]]= 0
    image[height-bnd[1]:height,:]=0
    image[:,bnd[3]:width]=0
    return(image)
...
def minZ(focalLength, Baseline): #focal length in pixles, baseline in mm
    return(focalLength*Baseline/126)
